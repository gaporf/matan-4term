...
\part{20.04.2020}
    ...
    \subsection{Формула Стокса}
        ...
        $(P, Q, R)$~--- гладкое векторное поле в $U(\Omega)$.
        
        Тогда $\int\limits_{\partial \Omega} P dx + Q dy + R dz = \iint\limits_{\Omega} (R'_y - Q'_z) dy dz + (P'_z - R'_x) dzdx + (Q'_x - P'_y) dxdy$.
        ...
        \subsubsection{Доказательство}
            ...
            $\int\limits_L P x'_u du + P x'_v dv = \iint\limits_G \dfrac{\partial}{\partial u} (P x'_v) - \dfrac{\partial}{\partial v} (P x'_u) du dv = \iint \left( P'_x x'_u + P'_y y'_u + P'_z z'_u \right) x'_v + P x''_vv - \left(P'_x x'_v + P'_y y'_v + P'_y z'_v \right) x'_u - P x''_uv du dv = \iint P'_x \mathbf{0} + P'_y (x'_v y'_u - x'_u y'_v) + P'_z (x'_v z'_u - x'_u z'_v) = \iint\limits_{G}$
            
            Получили что хотели.
        
        \subsubsection{Какие-то вычисления}
        
            $d P dx + d Q dy + dRdz$, $dxdx = 0$, $dxdy = -dydx$.
        
            $(P'_x dx + P'_y dy + P'_z dz)dx + \ldots - P'_ydxdy + P'_z dzdx + \ldots$.

\newpage
    
\part{Гильбертовы пространства}

    $V \in \mathbb{R}$, $\mathbb{C}$.
    
    $\langle \cdot, \cdot \rangle : V \rightarrow \mathbb{R}$.
    
    $\| v \| = \sqrt{\langle v, v \rangle}$
    
    $\left| \langle v, u \rangle \right| \leqslant \| u \| \cdot \| v \|$.
    
    \subsection{Определение}
    
        $\mathcal{H}$~--- линейное пространство со скалярным произведением (и соответствующей нормой) полное (как линейное нормированное пространство)~--- Гильбертово пространство.
    ...
    \subsection{Теорема о свойствах сходимости в Гильбертовом пространстве}
        ...
        \begin{enumerate}
            ...
            \item $\sum x_k$~--- отрог. ряд. Тогда $\sum x_k$~--- сходится $\Leftrightarrow \sum \| x_k \|^2 < +\infty$ и при этом $\| \sum x_k \|^2 = \sum \| x_k \|^2$.
    ...
    \subsection{Определение}
        ...
        Ортогональная система (семейство) векторов~--- $\left\{ e_k \right\}$, $e_k \in \mathcal{H}$, что $\forall i, j : i \neq j : e_i \perp e_j$, $e_k \neq 0$. 
