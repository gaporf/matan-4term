...
\part{23.03.2020}
    ...
    У нас давно не было занятий $\smiley{}$:
    ...
    \subsection{Теорема (самая лучшая теорема, которую и доказывать не надо)}
        ...
        \subsection{Примеры}
            ...
            \begin{enumerate}
                ...
                \item Полярные координаты: $\mathbb{R}^2$, $x = r \cos \varphi$, $y = r \sin \varphi$, тогда $\mathcal{I} = r$.
                
                \item Цилиндрический координаты: $(r, \varphi, z)$, тогда
                ...
                \item Сферические координаты в $\mathbb{R}^3$: 
                    ...
                    Сферические координаты в $\mathbb{R}^m$:
                    
                    $r$~--- расстояние от центра до точки
                    
                    $\varphi_1$, $\varphi_2$, $\ldots$, $\varphi_{m - 1}$~--- углы.
                    ...
                    Потом преобразование интегралов (не понятно на видео)
                    
                    $\int dx_1 \ldots dx_m = \int \rho_{m - 1} dx_1 \ldots dx_{m - 2} d\rho_{m - 1} d \varphi_{m - 1} = \int \rho^2_{m - 2} \sin \varphi_{m - 2} dx1\ldots dx_{m - 3} d \rho_{m - 2} d \varphi_{m - 2} d \varphi_{m - 1} = \rho^3_{m - 3} \sin^2 \varphi_{m - 3} \sin \varphi_{m - 2} dx \ldots$
    ...
    \subsection{Теорема}
        ...
        $B(s, t) = \int\limits^1_) x^{s - 1} (1 - x)^{t - 1} dx = \dfrac{\Gamma(s) \Gamma(t)}{\Gamma (s + t)}$.
        ...
        \subsubsection{Доказательство}
            ...
            $\int\limits^{+\infty}_0 du \int\limits^u_0 dx x^{s - 1} (u - x)^{t - 1} e^{-u}$, 
    ...
    Пример объём шара в $\mathbb{R}^m$:
    
        $\lambda_m B(0, R) = \int\limits_{x_1^2 + \ldots + x_m^2 = R^2} 1 dx$, введём сферические координаты.
        
        $\int\limits^R_0 dr \int\limits^{\pi}_0d\varphi_1 \ldots \int\limits^{\pi}_0 d \varphi_{m - 2} \int\limits^{2 \pi}_0 d \varphi_{m - 1} r^{m - 1} \sin^{m - 2} \varphi_1, \sin^{m - 3} \varphi_2 \ldots \sin \varphi_{m - 2}$, а дальше берём бету-функцию и будем счастливы.
        
        $\int\limits^{\pi}_0 \left(\sin \varphi_k \right)^{m - 1 - k} = 2 \int\limits^{\pi / 2}_0 t^{\frac{m - 1 - k}{2} - \frac{1}{2}} (1 - t)^{-0.5} dt = B(\frac{m - k}{2}, \frac{1}{2}) =\dfrac{\Gamma\left( \frac{m - k}{2} \right) \Gamma\left( \frac{1}{2} \right)}{\Gamma \left( \frac{m - k}{2} + \frac{1}{2} \right)}$
...
\newpage
    ...
    $H(t) := \mu X(h < t)$~--- функция распределения функции $h$ по $\mu$.
    
    $(H : \mathbb{R} \rightarrow \mathbb{R})$.
    
    Очевидно, что $H$ возрастает.
    
    $h : X \rightarrow \overline{\mathbb{R}}, \nu := h(\mu)$.
    
    $\nu(A) = \mu \left( h^{-1}(A) \right)$
    
    Факт, что $h$~--- измеримая.
    
    Тогда $\forall B \in \mathcal{B} \left( \mathbb{R} \right)$, $h^{-1} \left( \mathcal{B} \right)$~--- измеримая.
    
    $\mu_H[a, b) = H(b - 0) - H(a - 0)$~--- мера Бореля-Стилтьеса.
    ...
    \subsection{Лемма}
        ...
        Если потрудитесь хоть как-то расшифровать, то получится, что написана тут тривиальность.
        
        \subsection{Доказательство}
        
            $\mu_h[a, gg) = H(b - 0) - H(a - 0) = H(b) - H(a)$~--- непрерывность меры снизу.
...
\newpage
    ...
    \begin{enumerate}
        ...
        \item $(X, \mathcal{A}, \mu)$, $f : X \rightarrow \mathbb{C}$, $f(x) = g(x) + i h(x)$.
            ...
            $f$~--- измерима $\Leftrightarrow g = \Re f$ и $h = \Im f$~--- измеримая.
            
            $f$~--- суммируемая $\Leftrightarrow g = \Re f$ и $h = \Im f$~--- суммируемые.
        ...
        \item Неравенство Гёльдера:
            ...
            $p$, $q > q$, $\frac{1}{p} + \frac{1}{q} = 1$
            
            заданы почти везде, измеримы.
            
            $(X, \mathcal{A}, \mu)$, $f$, $g : X \rightarrow \mathbb{C}$ ($\mathbb{R}$)
            
            Тогда $\int\limits_X |fg| d mu \leqslant \left( \int\limits_x |f|^p \right)^{1/p} \left( \int\limits_X |f|^q \right)^{1/q}$
            
            Ступенчатые функции и Леви.
            
        \item Неравенство Минковского
        
            $(X, \mathcal{A}, \mu)$, $f$, $g : X \rightarrow \mathbb{C}$~--- измерима почти везде, конечна, $1 \leqslant p < +\infty$.
            
            Тогда $\left( \int\limits_X |f + g|^p \right)^{1/p} \leqslant \left( \int\limits_x |f|^p \right)^{1/p} + \left( \int\limits_X |g|^q \right)^{1/q}$
